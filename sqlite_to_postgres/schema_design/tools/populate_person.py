...
with contextlib.closing(psycopg2.connect(**dsn)) as conn, conn.cursor() as cur:
    ...
    query = """ INSERT INTO person (
                ...
                VALUES (%s, %s, %s, %s)"""
    query_data = [(pk, fake.last_name(), now, now) for pk in persons_ids]
    execute_batch(cur, query, query_data, page_size=PAGE_SIZE)
    conn.commit()
    ...
    film_works_ids = [id_data[0] for id_data in cur.fetchall()]
